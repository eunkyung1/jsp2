...
<html>
	<head>
		...
		<title>회원정보 수정</title>
		<style>
		.txtOn{color: orange; font-weight:700;}
		.txtOff{color: blue; font-weight:700;}
		</style>
		<script>
			$(function(){
				var idConfirm = 0;
				...
				$("#insertBtn").click(function(){
					...
					//var idPtn= /^[a-zA-Z0-9]{4,16}$/; //영문소문자 2자리~5자리 사이 값을 비교 패턴
					var idPtn= /^[a-z]{1}[a-zA-Z0-9]{2,10}$/; //총 16자리 작성가능, 첫글자는 소문자이면서 다음부터 소대문자,숫자 가능
					//var namePtn= /^[가-힣]{1,4}$/; //국문 1~4자리까지 가능
					//var pwPtn= /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*<>?]).{3,}$/; // 영문자,숫자,특수문자 1개이상 적어야 함
					//var pwPtn= /^[가-힣]{1,4}$/; //국문 1~4자리까지 가능
					//var f_tellPtn= /^[0-9]{3}$/; //유선전화 가능 숫자 0,1로 3자리만 가능
					//var m_tellPtn= /^[0-9]{3,4}$/; //숫자 0~9 3~4자리로 가능
					//svar l_tellPtn= /^[0-9]{3,4}$/; //숫자 0~9 3~4자리로 가능
					if(!idPtn.test(id)){
						alert("영문과 숫자 2~10자리까지 입력이 가능합니다.");
						return false;
					//if(!pwPtn.test(pw)){
